...
class EnvLoader
{
    private static function get_env_file_name(
        string $dir,
    ...
    public static function get_env(
        string $dir,
        ?string $env_class = null,
        ?string $environment = null,
    ) {
        $env_file = static::get_env_file_name($dir);
        $env = static::get_vars_from_file_name($env_file);

        // If a environment is set, get extra variables
        if ($environment !== null || isset($env["env"])) {
            $environment ??= $env["env"];
            $env_environment_file = static::get_env_file_name(
                $dir,
                $environment,
            );
            $env_environment = static::get_vars_from_file_name(
                $env_environment_file,
            );
            $env = [...$env, ...$env_environment];
        }
        ...
        if ($env_class !== null && class_exists($env_class)) {
            ...
            foreach ($rc->getProperties() as $prop) {
                $env_var = $env[$prop->getName()] ?? null;
                if ($env_var === null && !$prop->hasDefaultValue()) {
                    throw new Exception(
                        "Missing environment variable: " . $prop->getName(),
                    );
                }
